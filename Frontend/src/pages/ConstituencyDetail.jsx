...
const ConstituencyDetail = () => {
  ...
  const decodedConstituency = decodeURIComponent(constituencyName || "");
    ...
    const load = async () => {
      try {
        ...
        const res = await getElectionResults({
          constituency: decodedConstituency,
          page,
          limit: 20,
          search,
          sortBy,
        ...
        setError(err.message || t.common.failedFetchConstituencyDetails);
      } finally {
        setLoading(false);
      }
    };
    load();
  }, [decodedConstituency, page, search, sortBy, sortOrder, t.common.failedFetchConstituencyDetails]);

  const details = useMemo(() => {
    const totalVotes = rows.reduce((sum, r) => sum + (r.votes || 0), 0);
    const partyCount = new Set(rows.map((r) => r.party).filter(Boolean)).size;
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <Link to="/constituencies" className="inline-flex text-sm text-[#1B2A4A] dark:text-cyan-300 hover:underline mb-4">
        &larr; {t.constituencies.title}
      </Link>

      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-5 mb-6 card-3d">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-slate-100">{decodedConstituency}</h2>
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="rounded-xl p-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
            <p className="text-xs text-slate-600 dark:text-slate-300">{t.common.party}</p>
            <p className="text-xl font-bold text-slate-800 dark:text-slate-100">{details.partyCount}</p>
          </div>
        ...
        {details.topCandidate && (
          <div className="mt-4 p-4 rounded-xl border border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-300">
              {details.topCandidate.party} • {details.topCandidate.district_name}
            </p>
          </div>
        )}
      </div>
              ...
              <div key={c._id || `${c.candidate_name}-${i}`} className="rounded-lg border border-gray-100 dark:border-slate-800 p-3 bg-gray-50 dark:bg-slate-800/50">
                <div className="flex justify-between items-start gap-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-slate-300">{c.district_name} • {c.province_name}</p>
                  </div>
